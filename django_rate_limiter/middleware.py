...
class RateLimitMiddleware(MiddlewareMixin):
    ...
    def _add_rate_limit_headers(self, response: HttpResponse, metadata: Dict[str, Any], limit: int):
    ...
    def process_response(self, request: HttpRequest, response: HttpResponse) -> HttpResponse:
        ...
        if hasattr(request, '_rate_limit_metadata'):
            self._add_rate_limit_headers(
                response,
                request._rate_limit_metadata,
                request._rate_limit_limit
            )
...
class PerIPRateLimitMiddleware(MiddlewareMixin):
    ...
    def __init__(self, get_response):
        ...
        if self.config:
            ...
            self.rate_limiter = get_rate_limiter(algorithm=algorithm, backend=self.backend)
...
class PerUserRateLimitMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request: HttpRequest) -> Optional[HttpResponse]:
        ...
        try:
            ...
            return JsonResponse({
                'error': 'Rate limit exceeded',
                'message': f'Too many requests from user',
                'retry_after': e.retry_after,
            }, status=429)
