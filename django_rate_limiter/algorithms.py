...
class BaseRateLimiter(ABC):
    ...
    @abstractmethod
    def is_allowed(self, identifier: str, limit: int, window: int, scope: str = "") -> Tuple[bool, Dict[str, Any]]:
        """
    ...
    def enforce(self, identifier: str, limit: int, window: int, scope: str = "") -> Dict[str, Any]:
        """
...
class SlidingWindowRateLimiter(BaseRateLimiter):
    ...
    def is_allowed(self, identifier: str, limit: int, window: int, scope: str = "") -> Tuple[bool, Dict[str, Any]]:
        ...
        def update_window(current_data):
            ...
            requests = [
                req_time for req_time in current_data.get("requests", [])
        ...
        if result and result.get("allowed", False):
            ...
            retry_after = max(0, int((oldest_request + window) - current_time)) if oldest_request else window
...
class TokenBucketRateLimiter(BaseRateLimiter):
    ...
    def is_allowed(self, identifier: str, limit: int, window: int, scope: str = "", 
                   tokens_per_request: int = 1, burst_capacity: int = None) -> Tuple[bool, Dict[str, Any]]:
        """
        ...
        def update_bucket(current_data):
            ...
            current_tokens = min(
                burst_capacity,
                current_data.get("tokens", 0) + tokens_to_add
            )
...
class SlidingWindowCounterRateLimiter(BaseRateLimiter):
    ...
    def __init__(self, backend: BaseBackend = None, key_prefix: str = "rate_limit", num_windows: int = 10):
    ...
    def is_allowed(self, identifier: str, limit: int, window: int, scope: str = "") -> Tuple[bool, Dict[str, Any]]:
        ...
        def update_counters(current_data):
            ...
            if total_count < limit:
                ...
                windows[str(current_sub_window)] = windows.get(str(current_sub_window), 0) + 1
                allowed = True
                total_count += 1
            else:
                allowed = False
...
def get_rate_limiter(algorithm: str = "sliding_window", **kwargs) -> BaseRateLimiter:
    ...
    Args:
        algorithm: Type of algorithm ("sliding_window", "token_bucket", "fixed_window", "sliding_counter")
