    ...
    for i in range(5):
        try:
            metadata = limiter.enforce(
                identifier="user123", limit=3, window=60, scope="demo"
            )
...
def demonstrate_token_bucket():
    ...
    for i in range(12):
        try:
            ...
            if allowed:
                print(
                    f"Request {i+1}: ALLOWED - Remaining tokens: {metadata.get('remaining_tokens', 0):.1f}"
                )
            else:
                print(
                    f"Request {i+1}: DENIED - Retry after: {metadata.get('retry_after', 0)} seconds"
                )
                break
...
def demonstrate_fixed_window():
    ...
    for i in range(6):
        try:
            allowed, metadata = limiter.is_allowed(
                identifier="user789", limit=4, window=60, scope="demo"
            )
            if allowed:
                print(
...
def demonstrate_programmatic_usage():
    ...
    for i in range(3):
        try:
            ...
            print(
                f"API Request {i+1}: SUCCESS - Remaining: {metadata.get('remaining', 0)}"
            )
        except RateLimitExceeded as e:
